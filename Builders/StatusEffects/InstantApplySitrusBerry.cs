...
[UsedImplicitly]
public class InstantApplySitrusBerry : IStatusBuilder
{
    ...
    public DataFileBuilder<StatusEffectData, StatusEffectDataBuilder> Builder()
    {
        return new StatusEffectDataBuilder(Mod.Instance)
            ...
            .WithText("The target gains When hit, restore <keyword=health> by <{a}>")
            .WithStackable(true)
            .WithCanBeBoosted(false)
            .SubscribeToAfterAllBuildEvent<StatusEffectInstantApplyEffectAndUpdate>(status =>
            {
                status.effectToApply = Mod.GetStatus("When Hit Heal Self");
            });
    }
}