...
public class CardRewardSequence : UISequence
{
  ...
  public void AddCards(CardContainer from)
  {
    ...
    foreach (var card in (IEnumerable<CardPocketSequence.Card>) this.cards.OrderBy<CardPocketSequence.Card, float>((Func<CardPocketSequence.Card, float>) (_ => PettyRandom.Range(0.0f, 1f))))
    {
      card.Take();
      var hover = card.entity.display?.hover;
      if (hover != null)
      {
  ...
  public override IEnumerator Run()
  {
    ...
    foreach (var entity in (IEnumerable<Entity>) this.container.OrderBy<Entity, float>((Func<Entity, float>) (_ => PettyRandom.Range(0.0f, 1f))))
    {
      clump.Add(this.MoveCard(entity));
      yield return (object) Sequences.Wait(this.shuffleDelay.PettyRandom());
    }
    this.backButton.SetActive(true);
    yield return (object) clump.WaitForEnd();
    this.cardController.enabled = true;
    yield return (object) new WaitUntil(new Func<bool>(this.PromptEnd));
    this.promptEnd = false;
    ...
    yield return (object) this.ReturnCards();
  ...
  {
    if (!((Object) entity.transform.parent == (Object) this.container.holder))
    {
      return;
    }
    var childPosition = this.container.GetChildPosition(entity);
    var childRotation = this.container.GetChildRotation(entity);
    var childScale = this.container.GetChildScale(entity);
    if (includeRandomness)
    {
      var a = PettyRandom.Vector3();
  ...
  [CompilerGenerated]
  public bool PromptEnd() => this.promptEnd;
  ...
  public class Card
  {
    ...
    public void Return()
    {
      this.entity.RemoveFromContainers();
      this.entity.enabled = this.preEnabled;
      foreach (var preContainer in this.preContainers)
      {
        preContainer.Add(this.entity);
      }
      var hover = this.entity.display?.hover;
      if (hover != null)
      {
